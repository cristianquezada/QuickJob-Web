...
<?php 
$sql_consulta="SELECT IdProyecto,NombreProyecto FROM proyecto where fkjefeproyecto='$id'";
		$result_consulta=mysql_query($sql_consulta);
		  while($Datos=mysql_fetch_array($result_consulta))
		{
			
			$idpro=$Datos['IdProyecto'];
$res=mysql_query("SELECT count(*) as solicitudes FROM solicitud  WHERE fkproyecto='$idpro'");
		$dat=mysql_fetch_assoc($res);
		
		$resultado=mysql_query("SELECT count(*) as total FROM solicitud where estadoSolicitud='5' and fkproyecto='$idpro'");
		$dato=mysql_fetch_assoc($resultado);

		
		$quedan=$dat['solicitudes']-$dato['total'];
    ...
    <div>Total:  <?php echo $dat['solicitudes'];?> Solicitudes</div>
...
var valores={
	"Personas contratadas: <?php echo $dato['total']; ?>":{valor:<?php echo $dato['total'];?>,color:"coral"},
	"Solicitudes realizadas: <?php echo $quedan;?>":{valor:<?php echo $quedan;?>,color:"gray"},
}
