...
$res2=mysql_query("SELECT * FROM proyecto where Idproyecto='$codigo'");
$dato2=mysql_fetch_assoc($res2);
$nombreProyecto=$dato2['NombreProyecto'];
...
<br><br><br>
    ...
    <h1 align="center">Selección de Personal proyecto: <?php echo "<br>".$nombreProyecto; ?></h1>
    <div class="col-md-8 col-md-offset-2 myform-cont">
   <form action="envioPreseleccion.php" method="POST" name="formPreseleccion" onsubmit="return pregunta()"> 
   <div class="myform-bottom">
	    ...
	    $sql_consulta="SELECT t.idTrabajador, t.nombreCompleto, t.direccion, t.fono,t.venExamenMed, t.venInd,t.bloqueo, s.fkproyecto, u.rut,c.cargo FROM trabajador t INNER JOIN cargos c ON c.idCargo=t.fkCargo INNER JOIN ciudad ciu ON ciu.idCiudad=t.fkCiudad INNER JOIN solicitud s ON t.idTrabajador = s.fktrabajador INNER JOIN usuario u ON t.fkusuario = u.idusuario WHERE  s.estadoSolicitud = '2' AND s.fkproyecto = '".$codigo."'";
...
$(document).ready(function() {
    $('#example').DataTable( {
        "lengthMenu": [[-1], ["Todo"]]
                ...
                <button type="submit"  value="" class="btn btn-success">Confirmar participación</button>
</form>
              </div>
            </div>
            </body>
...
function pregunta(){
    ...
    if(confirm("¿Está seguro de confirmar al personal seleccionado?"))
    {
